...
/*function check(name, sum) {
    ...
    //Since, sum was already 1 (previous alloted). If all the buttons are pressed and it does not go in the if condition therefore we can say it is a draw, as 9 buttons are pressed.
    else if (sum == 10) {
        socket.emit("gameOver", { name: name })
...
function check(name, sum) {
    ...
    function addStrikeout(btns) {
        btns.forEach(btn => document.getElementById(btn).classList.add("strikeout"));
    }
